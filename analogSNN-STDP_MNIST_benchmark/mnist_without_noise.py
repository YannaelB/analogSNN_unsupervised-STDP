...
Example usage:
    acc = main_mnist_without_noise(0,'without_noise_final',noise=False)
...
from mnist_analog_eval import evaluation_noiseless_and_analog
...
class SNN_eNeuron():
    def __init__(self, debug=False):
        ...
        model['input_synapse_exci'].delay = 'rand()*0*us' # Not used here
    ...
    def final_evaluation(self,idx_epoch,X_test,y_test,noise=False,plot=False):
        ...
        return accuracy_max,confusion_matrix,weight_exci,weight_inhi, weight_wta,self.labeled_neurons
...
def main_mnist_without_noise(iterat,dir_name,noise=False):
    '''
    Runs a simulation of an SNN on the MNIST dataset using eNeuron from scenario 3 (+ testing on scenario 3.1 and 3.2)
    ...
    debug = False
    noise = False #Unactivate noise (scenario 3)
    ...
    accuracy_max,labeled_neurons,spikes,weight_exci,weight_inhi,weight_wta = model.intermediate_evaluation(-1,X_test_batches[-1], y_test_batches[-1],noise=False,plot=debug)
    accuracy_evol.append(accuracy_max)
    print(" FINISH noiseless pre-epoch with accuracy = ", accuracy_max," in ",time.time()-tic, "s \n")
    ...
    for idx_epoch in range(epoch_max):
        ...
        accuracy_max,labeled_neurons,spikes,weight_exci,weight_inhi,weight_wta = model.intermediate_evaluation(idx_epoch,X_test_batches[idx_epoch], y_test_batches[idx_epoch],noise=False,plot=debug)
        accuracy_evol.append(accuracy_max)
        save_accuracy_txtfile_short(f"data_backup_test/{directory}/Accuracy_evolution_save.txt",np.array(accuracy_evol))
        #weight_evolution(directory,weight_evol_exci,weight_evol_inhi)
        print(" FINISH noiseless epoch ", idx_epoch," with accuracy = ",accuracy_max," in ",time.time()-tic, "s \n")
        tic = time.time()
    ...
    accuracy_max,confusion_matrix,weight_exci,weight_inhi,weight_wta,labeled_neurons = model.final_evaluation(idx_epoch,X_test_batches[0], y_test_batches[0],noise=False,plot=debug)
    ...
    csv_save(f'data_backup_test/{directory}/saving_accuracy_eval_noiseless.csv',[accuracy_max])



    ### Evaluation post-training with constant standart deviation noise (scenario 3.1)
    X_train_batches, y_train_batches, X_test_batches, y_test_batches, nb_batch = load_mnist(nb_train=5000, nb_test=5000, working_range=3, batch_size_train=5000,batch_size_test=5000)
    accuracy_max,confusion_matrix,weight_exci,weight_inhi, weight_wta,labeled_neurons = model.final_evaluation(idx_epoch,X_test_batches[0], y_test_batches[0],noise=True,plot=debug)        
    df_cm = pd.DataFrame(confusion_matrix, range(10), range(10))
    plt.figure(figsize=(10,7))
    plt.title("Confusion matrix")
    ...
    plt.xlabel('Predicted label')
    plt.ylabel('True label')
    plt.savefig(f"data_backup_test/{directory}/{directory+str(itera)}/confusion_matrix_noise_cst.png")
    csv_save(f'data_backup_test/{directory}/saving_accuracy_eval_noise_cst.csv',[accuracy_max])


    ### Evaluation post-training with analog standart deviation noise (scenario 3.2)
    X_train_batches, y_train_batches, X_test_batches, y_test_batches, nb_batch = load_mnist(nb_train=5000, nb_test=5000, working_range=3, batch_size_train=5000,batch_size_test=5000)
    accuracy_max,confusion_matrix = evaluation_noiseless_and_analog(labeled_neurons,weight_exci,weight_inhi,weight_wta,X_test_batches[0], y_test_batches[0])    
    df_cm = pd.DataFrame(confusion_matrix, range(10), range(10))
    plt.figure(figsize=(10,7))
    plt.title("Confusion matrix")
    sn.set(font_scale=1.1) # label size
    ...
    plt.xlabel('Predicted label')
    plt.ylabel('True label')
    plt.savefig(f"data_backup_test/{directory}/{directory+str(itera)}/confusion_matrix_noise_analog.png")
