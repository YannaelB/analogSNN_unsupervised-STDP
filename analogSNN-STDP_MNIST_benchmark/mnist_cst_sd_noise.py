...
Example usage:
    acc = main_mnist_cst_sd_noise(0,'cst_noise_final',noise=True)
# --------------------------------------------------------------
'''
...
def main_mnist_cst_sd_noise(iterat,dir_name,noise=False):
    '''
    Runs a simulation of an SNN on the MNIST dataset using eNeuron from scenario 1
    ...
    # Creating the analog SNN model
    debug = False
    noise = True  # activate random noise (scenario 1)
    ...
    accuracy_max,labeled_neurons,spikes,weight_exci,weight_inhi,weight_wta = model.intermediate_evaluation(-1,X_test_batches[-1], y_test_batches[-1],noise=noise,plot=debug)
    accuracy_evol.append(accuracy_max)
    print(" FINISH cst pre-epoch with accuracy = ", accuracy_max," in ",time.time()-tic, "s \n")
    ...
    for idx_epoch in range(epoch_max):
        ...
        accuracy_max,labeled_neurons,spikes,weight_exci,weight_inhi,weight_wta = model.intermediate_evaluation(idx_epoch,X_test_batches[idx_epoch], y_test_batches[idx_epoch],noise=noise,plot=debug)
        accuracy_evol.append(accuracy_max)
        save_accuracy_txtfile_short(f"data_backup_test/{directory}/Accuracy_evolution_save.txt",np.array(accuracy_evol))
        print(" FINISH cst_noise epoch ", idx_epoch," with accuracy = ",accuracy_max," in ",time.time()-tic, "s \n")
        tic = time.time()
    ...
    accuracy_max,confusion_matrix = model.final_evaluation(idx_epoch,X_test_batches[0], y_test_batches[0],noise=noise,plot=debug)
