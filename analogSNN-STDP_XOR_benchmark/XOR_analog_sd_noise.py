...
Example usage:
    assessing_analog_sd_noise(0,'analog_noise',noise=True)
...
dv/dt = (v_rest - v) / tau2 + redress*((I_sin*I0/(a_value*Hz))*Rm)/ tau : volt (unless refractory)
I_total = clip(Ie + I_inhi + I, 0*nA, 15*nA) : amp
I_sin = y_interp_function(I_total) : Hz (constant over dt)
...
threshold = v_threshold + int(noise_activity)*sigma*randn()*(v_threshold-v_reset)*I_sin*sqrt(idx_spk) : volt (constant over dt)
sigma = int(I_sin>frequency_min)*sqrt(scale*q/(I_total+avoid_error)*(1/(I_sin+frequency_min) + R_ds*C_m/pi)) : second (constant over dt) #standart deviation
idx_spk : 1
...
def assessing_analog_sd_noise(iterat,dir_name,noise=True):
    '''
    Runs a simulation of an analog SNN on the MNIST dataset using eNeuron from scenario 2
    ...
    model.labelisation(0,X_train,y_train,noise,plot=debug)
    ...
    for idx_epoch in range(epoch_max):
        ...
        # Creating batch data
        X_train,y_train,X_test,y_test = batch_data(n_train,n_test)
        model.training(idx_epoch,X_train,y_train,noise,plot=debug)
        model.labelisation(idx_epoch,X_train,y_train,noise,plot=debug)
    ...
    print(" DONE with assessing_analog_sd_noise with noise = ", noise, "in ",time.time()-tic, "s \n")
