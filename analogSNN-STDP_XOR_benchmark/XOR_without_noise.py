...
The analog SNN is trained with noiseless neurons (SCENARIO 3 in the paper)
...
Example usage:
    assessing_without_noise(0,'without_noise',noise=False)
...
from XOR_analog_eval import evaluation_noiseless_and_analog
...
class SNN_eNeuron():
    def __init__(self, debug=False):
        '''
            Initialization of the analog SNN including setting up neuron groups and synapses
        '''
    ...
    def evaluation(self,idx_epoch,X_test,y_test,noise=False,plot=False):
        '''
            Function for testing the network on the test dataset and calculates the training/testing accuracy
        '''
    ...
    def plot_3D_XOR(self,idx_epoch,n_point=5,noise=False,plot=False):
        '''
...
def assessing_without_noise(iterat,dir_name,noise=False):
    '''
    Runs a simulation of an analog SNN on the XOR problem using eNeuron from scenario 3 (+ testing on scenario 3.1 and 3.2)
    ...
    # We pick n_test=2 but n_test=10 would give the same results because this analog SNN is fully deterministic (noiseless)
    # Hence the SNN always spikes the exact same way for a given input, and there are only 4 different inputs
    ...
    # The initial predictions are expected to be random, resulting in an accuracy of approximately 50%
    X_train,y_train,X_test,y_test = batch_data(n_train,n_test)
    model.labelisation(0,X_train,y_train,noise=False,plot=debug)
    accuracy_max,weight_evol_exci,weight_evol_inhi,labeled_neurons2,weight_exci,weight_inhi,weight_wta = model.evaluation(0,X_test,y_test,noise=False,plot=debug)
    ...
    for idx_epoch in range(epoch_max):
        ...
        X_train,y_train,X_test,y_test = batch_data(n_train,n_test)
        model.training(idx_epoch,X_train,y_train,noise=False,plot=debug)
        model.labelisation(idx_epoch,X_train,y_train,noise=False,plot=debug)
        accuracy_max,weight_evol_exci,weight_evol_inhi,labeled_neurons2,weight_exci,weight_inhi,weight_wta = model.evaluation(idx_epoch,X_test,y_test,noise=False,plot=debug)
        accuracy_evol.append(accuracy_max)
    ...
    X_3D,Y_3D,Z_3D = model.plot_3D_XOR(idx_epoch,n_point=10,noise=False,plot=False)
    csv_save(f'data_backup_test/{directory}/saving_Z_3D_without_noise.csv',Z_3D)


    ### Evaluation post-training with constant standart deviation noise (scenario 3.1)
    X_train,y_train,X_test,y_test = batch_data(n_train=25,n_test=25)
    ...
    csv_save(f'data_backup_test/{directory}/saving_accuracy_cst_sd_noise.csv',[accuracy_max])
    #3D figure
    X_3D,Y_3D,Z_3D = model.plot_3D_XOR(idx_epoch,n_point=10,noise=True,plot=False)
    csv_save(f'data_backup_test/{directory}/saving_Z_3D_cst_noise.csv',Z_3D)


    ### Evaluation post-training with analog standart deviation noise (scenario 3.2)
    accuracy_max,Z_3D = evaluation_noiseless_and_analog(labeled_neurons2,weight_exci,weight_inhi,weight_wta)
    csv_save(f'data_backup_test/{directory}/saving_accuracy_analog_sd_noise.csv',[accuracy_max])
    csv_save(f'data_backup_test/{directory}/saving_Z_3D_ananlog_noise.csv',Z_3D)

    

    print(" DONE with assessing_without_noise in ",time.time()-tic, "s \n")
