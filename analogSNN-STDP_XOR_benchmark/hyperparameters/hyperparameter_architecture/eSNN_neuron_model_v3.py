...
# This model use a variable Resistance and a log approximation
V_a, V_b= 70*mV,90*mV
...
class SNN_eNeuron():
    def __init__(self, debug=False):
        ...
        model['input'] = NeuronGroup(N=n_input, model=neuron_eqs_v3, threshold='v >= v_threshold + v_std_thresh*randn()', reset='v = v_reset', refractory='refrac', method='heun',name='input')
        model['input'].inhi_factor = 1
        model['input'].activity = 1

        print("n_output = ",n_output)
        model['output'] = NeuronGroup(N=n_output, model=neuron_eqs_v3, threshold='v >= v_threshold + v_std_thresh*randn()', reset='v = v_reset', refractory='refrac', method='heun',name='output')
...
directory = "hyperparams_neuron_model_v3_backup"
def assessing_neuron_model_v3(itera):
    '''
