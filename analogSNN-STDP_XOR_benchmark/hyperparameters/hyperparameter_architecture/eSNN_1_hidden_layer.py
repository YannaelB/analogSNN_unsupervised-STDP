...
# This model utilises a constant Resistance and a log approximation
neuron_eqs_v1 = '''
...
n_input, n_hidden, n_output = 2,7,6
...
class SNN_eNeuron():
    def __init__(self, debug=False):
        ...
        model['hidden'] = NeuronGroup(N=n_hidden, model=neuron_eqs_v1, threshold='v >= v_threshold + v_std_thresh*randn()', reset='v = v_reset', refractory='refrac', method='heun',name='hidden')
        model['hidden'].inhi_factor = 1
        model['hidden'].activity = 1
        ...
        model['input_synapse_exci'] = Synapses(model['input'], model['hidden'], model=stdp_eqs_exci, on_pre=pre_eqs_exci, on_post=post_eqs_exci,name='input_synapse_exci')
        ...
        model['hidden_synapse_exci'] = Synapses(model['hidden'], model['output'], model=stdp_eqs_exci, on_pre=pre_eqs_exci, on_post=post_eqs_exci,name='hidden_synapse_exci')
        model['hidden_synapse_exci'].connect(True)
        model['hidden_synapse_exci'].lr = 0.5
        model['hidden_synapse_exci'].alpha = alpha_decay
        model['hidden_synapse_exci'].w = 'rand() * wmax * 0.8 + 0.1*wmax'
        model['hidden_synapse_exci'].delay = 'rand()*0*us'
        ...
        print("You've created :",len(model['input_synapse_exci'])+len(model['hidden_synapse_exci'])+len(model['wta_synapse']) , "synapses in your network")

        self.net = Network(model)
        
        self.weight_evol_input = []
        self.weight_evol_hidden = []
    ...
    def training(self,idx_epoch,X_train,y_train,plot=False):
        ...
        self.net['hidden_synapse_exci'].lr = 1
        ...
        for j, (sample, label) in enumerate(zip(X_train, y_train)):
            ...
            weight_matrix[self.net['hidden_synapse_exci'].i, self.net['hidden_synapse_exci'].j] = self.net['hidden_synapse_exci'].w
            sumCol = np.sum(weight_matrix, axis=0)
            colFactors = 1.5/sumCol
            for jj in range(n_output):
                weight_matrix[:,jj] *= colFactors[jj]
            self.net['hidden_synapse_exci'].w = weight_matrix[self.net['hidden_synapse_exci'].i, self.net['hidden_synapse_exci'].j]

        # Observation weight evolution
        weight = [self.net['input_synapse_exci'].w[i] for i in range(len(self.net['input_synapse_exci'].w))]
        self.weight_evol_input.append(weight)
        weight = [self.net['hidden_synapse_exci'].w[i] for i in range(len(self.net['hidden_synapse_exci'].w))]
        self.weight_evol_hidden.append(weight)
    ...
    def labelisation(self,idx_epoch,X_train,y_train,plot=False):
        ...
        self.net['hidden_synapse_exci'].lr = 0
    ...
    def evaluation(self,idx_epoch,X_test,y_test,plot=False):
        ...
        return accuracy_max,accuracy_mean,accuracy_temporal,accuracy_2,accuracy_4, self.weight_evol_input, self.weight_evol_hidden



directory = "hyperparams_hidden_backup"
def assessing_1_hidden_layer(itera):
    '''
    ...
    model.labelisation(0,X_train,y_train,plot=debug)
    accuracy_max,accuracy_mean,accuracy_temporal,accuracy_2,accuracy_4,weight_evol_exci,weight_evol_inhi = model.evaluation(0,X_test,y_test,plot=debug)
    ...
    print("done hidden_hyperparams in idx_epoch = ", idx_epoch+1, "epochs for accuracy = ", accuracy_max)
    ...
    plot_accuracy_evol(directory,accuracy_evol,n_output)

    return idx_epoch+1


#assessing_1_hidden_layer(1)